// Shim client to emulate the former `base44` SDK on top of our REST `api` client
// This lets existing code keep its imports while we talk to our backend.
...
function sortByKey<T extends Record<string, any>>(items: T[], order?: OrderKey): T[] {
  ...
  return [...items].sort((a, b) => {
    ...
    const bv = b?.[key];
    if (av === bv) return 0;
    // handle ISO dates or plain values
